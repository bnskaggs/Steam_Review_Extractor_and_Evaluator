...
def search_reviews(request: SearchRequest, db: Annotated[Database, Depends(get_db)] = Depends(get_db)):
...
def ask_reviews(request: AskRequest, db: Annotated[Database, Depends(get_db)] = Depends(get_db)):
    hits = semantic_search(
        query=request.query,
        app_id=request.app_id,
        start=request.date_start,
